function lab_04
    ...
    G_high = gaussfilt(4,20,'high');
    BB_high = buttfilt(6,20,'high');
    ...
    figure(4)
    plot(t,x0,t,filtfilt(G_high,1,x1),t,filtfilt(G_high,1,x2));
    title('Гауссовский фильтр');
    legend('Без помех','Помеха по Гауссу','Импульсная помеха');

    figure(5)
    plot(t,x0,t,filtfilt(BB_high,1,x1),t,filtfilt(BB_high,1,x2));
    title('Фильтр Баттеруорта');
    legend('Без помех','Помеха по Гауссу','Импульсная помеха');
end
